"""Tests for ``wait_for_vector_index_online``.
...
def _driver_yielding_states(*states: dict[str, object] | None) -> MagicMock:
    ...
    session_results: list[MagicMock | None] = []
    for state in states:
...
class TestWaitForVectorIndexOnline:
    ...
    def test_timeout_with_no_state_observed_carries_none(self) -> None:
        # Index never becomes visible — last_state stays None, error
        # carries that as a diagnostic.
    ...
    def test_error_message_contains_diagnostics(self) -> None:
        driver = _driver_yielding_states({"state": "FAILED", "populationPercent": 42.5})
        with pytest.raises(VectorIndexNotOnlineError) as excinfo:
            wait_for_vector_index_online(
                driver,
                database="neo4j",
