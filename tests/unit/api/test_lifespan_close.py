...
Load-bearing regression pin: it asserts the lifespan exit path actually
fires ``registry.close()``, which is what guarantees backend connections
release on uvicorn shutdown. Patches ``StoreRegistry.from_config_dir`` so
no real backend is touched.
"""
...
def test_lifespan_calls_registry_close_on_shutdown() -> None:
    ...
    with patch.object(
        ...
        with TestClient(app) as client:
            ...
            # Hit a route to confirm the app is alive.
