"""Tests for the capture sweep — env wiring, judge construction, strict mode.

``sweep.py`` reads every ``TRELLIS_CAPTURE_*`` env var and builds the judge
client, and it is the code path behind all three front doors (``python -m``,
the ``trellis-session-capture`` console script, and ``trellis worker
capture-sessions``). The failure mode it guards is specific: distillation
fail-closes on a missing client, so a misconfigured ``llm:`` block used to
produce a sweep that judged nothing, wrote nothing, and exited 0.
"""
...
class TestStrictMode:
    def test_defaults_to_strict(self, monkeypatch) -> None:
        monkeypatch.delenv("TRELLIS_CAPTURE_STRICT", raising=False)
        assert capture_sweep.strict_mode() is True

    @pytest.mark.parametrize("raw", ["0", "false", "FALSE", "no", "off"])
    def test_opt_out_values_disable_strict(self, monkeypatch, raw) -> None:
        monkeypatch.setenv("TRELLIS_CAPTURE_STRICT", raw)
        assert capture_sweep.strict_mode() is False

    @pytest.mark.parametrize("raw", ["1", "true", "yes", "anything-else"])
    def test_other_values_stay_strict(self, monkeypatch, raw) -> None:
        monkeypatch.setenv("TRELLIS_CAPTURE_STRICT", raw)
        assert capture_sweep.strict_mode() is True
...
class TestBuildJudgeClient:
    def test_returns_the_configured_client(self, registry) -> None:
        assert capture_sweep.build_judge_client(registry) is (
...
class TestJudgeUnavailableSessions:
    def test_counts_only_distill_unavailable_warnings(self) -> None:
        ...
        assert capture_sweep.judge_unavailable_sessions(report) == 2

    def test_clean_report_counts_zero(self) -> None:
        assert capture_sweep.judge_unavailable_sessions(_report()) == 0
...
class TestRunSweep:
    def test_env_vars_drive_the_sweep(self, monkeypatch, tmp_path, registry) -> None:
        ...
        monkeypatch.setattr(capture_sweep, "run_capture", spy)

        capture_sweep.run_sweep(registry=registry, dry_run=True)
    ...
    def test_defaults_apply_when_env_is_empty(
        ...
        capture_sweep.run_sweep(registry=registry)
    ...
    def test_builds_a_registry_when_none_is_injected(
        ...
        monkeypatch.setattr(
            capture_sweep.StoreRegistry, "from_config_dir", from_config_dir
        )
        spy = MagicMock(return_value=_report())
        monkeypatch.setattr(capture_sweep, "run_capture", spy)

        capture_sweep.run_sweep()
    ...
    def test_missing_judge_aborts_before_sweeping(self, monkeypatch, registry) -> None:
        ...
        with pytest.raises(capture_sweep.CaptureJudgeUnavailableError):
            capture_sweep.run_sweep(registry=registry)
