...
Tagging at write time is opt-in: set `TRELLIS_ENABLE_CLASSIFY_ON_INGEST=1` (also accepts `true`/`yes`/`on`) and five write seams tag as they store — bulk ingest (`trellis ingest corpus`, `ingest conversations`, the session-capture sweep), MCP `save_memory` and the evidence document `save_knowledge` creates, and REST `POST /documents` / `POST /evidence`. Fill-if-absent and fail-soft: existing tags are never clobbered, and a classification error stores the document untagged rather than failing the write. Everything else — rows stored before the flag was set, tags that have drifted, and the write paths outside those five seams — is covered by `trellis classify backfill`. Full contract: [operations.md — Document → content tags](docs/agent-guide/operations.md#document--content-tags-opt-in).
