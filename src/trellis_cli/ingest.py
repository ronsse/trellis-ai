...
def ingest_dbt_manifest(
    ...
    for entity in result.entities:
        ...
        if desc:
            ...
            # Through the seam (#360) so the classified bag reaches the vector
            # row too, the day one exists. Today it never does — nothing
            # embeds a `dbt:` document, so the mirror reads "absent" every
            # time; that is the embedding half of #568, still open.
            put_document(
                doc_store,
                vector_store,
                doc_id,
