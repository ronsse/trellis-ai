...
- **Sibling ADRs (written in parallel, this is their umbrella):**
  ...
  - [`./adr-egp-interop-bridge.md`](./adr-egp-interop-bridge.md) ([#220](https://github.com/ronsse/trellis-ai/issues/220)) — EGP as one governed projection/integration layer that interoperates with Trellis
...
Recent enterprise graph/ontology work surfaced a framing problem. When several graph-shaped proposals land in front of a builder at once — an Enterprise Graph Platform (EGP), a sportsbook/canonical entity store (SBK Entities), a curated domain ontology, a PROV-style practical-lineage spine, and Trellis / an agent context graph (fd-knowledge-graph) — the instinct is to ask "which one wins?" That question is wrong. None of them is a strict superset of the others. They differ on **capability fit, source authority, and operating layer**, and a healthy enterprise stack runs several of them at once, each owning the facts it is authoritative for.
...
1. **EGP-as-the-one-ontology.** A builder copies EGP's broad Node/Edge/Event shape into Trellis and treats it as *the* canonical model — closing the open-string type system, duplicating source-system payloads into the graph, and overwriting authoritative domain systems.
2. **Open strings with no contract.** A builder ignores EGP's governance strengths entirely and emits open strings with no profile, no projection, and no source-authority contract — so cross-domain questions have nothing governed to traverse.

This ADR is the umbrella. It establishes the capability map, the layer-ownership model, the comparison matrix, and the builder decision tree. The sibling ADRs go deep on individual pieces: optional governed **profiles** ([`./adr-ontology-profiles.md`](./adr-ontology-profiles.md)), the **promotion** path for query-history evidence ([`./adr-query-history-promotion.md`](./adr-query-history-promotion.md)), the **EGP interop bridge** ([`./adr-egp-interop-bridge.md`](./adr-egp-interop-bridge.md)), and **column/leaf guardrails** ([`./adr-column-leaf-modeling-guardrails.md`](./adr-column-leaf-modeling-guardrails.md)).
...
- It does not specify EGP's internals, the profile schema, or the promotion-review workflow — those are the sibling ADRs.
...
3. A **comparison matrix** (§5) — EGP / SBK Entities / domain ontology / practical lineage / Trellis-fdkg across eight dimensions.
...
The governing principles (§7) are: **Trellis core stays open-string and domain-neutral**; **EGP is one governed projection/integration layer among several**, not the canonical model everything else defers to; and **inferred / query-history facts are promoted through review before they become accepted graph facts** (the mechanism is [`./adr-query-history-promotion.md`](./adr-query-history-promotion.md)).

This ADR commits only to the framing and to linking it from the existing guides. It does not commit Trellis to building EGP, the profiles, or the promotion workflow — those are separately decided in the sibling ADRs.
...
|---|---|---|
| **Canonical identity & provider crosswalks** | Canonical entity store (SBK Entities / Sports Ontology) | Owns the blessed identifier for a real-world thing and the crosswalk between provider IDs. The authority that says "these three provider rows are the same team." |
...
| **Agent memory, retrieval feedback, observations** | Trellis / fd-knowledge-graph | Traces, context packs, behavioral evidence (`Observation` / `Measurement`), and the feedback loop. The graph of agent experience. |
| **Cross-domain enterprise graph projections** | EGP | Governed traversal *across* the domains the other layers own. A projection/integration layer, not a system of record. |
...
| **Governed assertion** | A reviewed, cross-domain claim asserted into a governed graph for traversal. Derived from systems of record under a governance contract. | No — a governed *projection* | EGP | Mutations gated by EGP's review/governance model. |
...
The five layers across eight dimensions. **This is deliberately not "EGP vs everything else."** EGP appears as one column — a governed projection/integration layer — alongside the others. Each layer is the natural owner of the rows where it scores "authoritative."

| Dimension | EGP | SBK Entities (canonical) | Domain ontology | Practical lineage | Trellis / fdkg |
|---|---|---|---|---|---|
...
Reading the matrix: no column dominates. EGP's strength is the **cross-domain traversal** row and the **governed assertion** stance; its weakness is that it is *not* a system of record for anything — it projects from the layers that are. Trellis's strength is the **agent-experience / retrieval** rows and the open-string formalism; its weakness is that it is a *serving index* for facts other layers own, so it must defer to them on authority.
...
3. **Use EGP-style projections for cross-domain questions** — described as one governed projection/integration layer that *interoperates with* the others, never as the canonical model. The bridge is [`./adr-egp-interop-bridge.md`](./adr-egp-interop-bridge.md).
...
- It states explicitly that EGP is **not** "better than everything else" — it is one governed projection/integration layer that interoperates with the other layers (§2, §4, §5, §8.3).
...
- **Not making EGP the only ontology.** EGP is one layer; the framing deliberately refuses to crown it.
...
- **Not specifying the profile schema, the promotion workflow, the EGP bridge protocol, or the column-guardrail thresholds.** Those are the four sibling ADRs (§1).
- **Nothing in this ADR is implemented.** It is a framing/design document; the artifacts it points to (profiles, promotion, EGP bridge) are separately proposed and not built.
