...
- **The advisory surfaces stopped overriding the map.** Amended 2026-09-04 ([#489](https://github.com/ronsse/trellis-ai/issues/489)). `trellis analyze generate-advisories`, `trellis analyze advisory-effectiveness` and `trellis worker curate` wrote `2` out at five call sites for a damaged or concurrently-written `advisories.json`, so *one condition had two codes*: those three exited `2` while `trellis policy list` exited `5` meeting the sibling `policies.json` damaged the same way. Both sides were locally sound — the advisory refusals were required to agree *with each other*, because a cron wrapper branches on the code without knowing which of them it hit — and the two rules collide at exactly one value, `2`, for the reason the amendment above gives. `EXIT_STORE` satisfies both. Worth recording *why* it was a five-line change: `StoreWriteRefusedError` is a `StoreError`, so `exit_code_for` answered `5` all along and these sites were overriding the canonical map rather than extending it. And two of the five were reached by neither named helper — `generate-advisories` raises inline in each `--format` arm, because its generator returns a report rather than raising — which is the shape a closure list derived by reading the helpers misses.
...
Per-command `except` blocks stay the right shape where a command has something specific to say. What #459 added underneath them is the *fallback*: `trellis_cli.main._BoundaryGroup.invoke` catches `TrellisError` at the root group, renders `exc.message` verbatim (the raiser already worded the path, the problem and the recovery command — a second vocabulary for the same facts is how `content_type` / `document_form` drifted apart in #325/#326), and exits `exit_code_for(exc)`. Under `--format json` / `--format jsonl` it emits the house `sanitized_error_payload` envelope instead, so the machine contract survives the failure path; the exit is below the format branch, as §3 requires. It catches narrowly: an untyped exception reaching there really is a `1`, and folding a `KeyError` into a tidy actionable envelope would turn a bug into an operator's problem to diagnose. One non-`TrellisError` class is named beside it — `PackAssemblyError` ([#493](https://github.com/ronsse/trellis-ai/issues/493)), which subclasses `RuntimeError` for reasons internal to retrieval but describes a *deployment condition*: every retrieval axis is down. It became CLI-reachable when [#488](https://github.com/ronsse/trellis-ai/issues/488) routed `trellis retrieve pack` through `PackBuilder`, and `trellis analyze pack-quality` had the same exposure, which is why the translation sits at the boundary rather than in either command. Reparenting it to `TrellisError` was considered and refused: `exit_code_for` resolves it to `EXIT_INTERNAL` either way, so the reparent buys the same envelope and the same code while changing what every `except TrellisError` in the tree catches — including `MutationExecutor`'s. A second such class gets a second named clause, visible in the diff.
