# ADR: Enterprise graph (EG) interop bridge
...
**Related:**
- [`./adr-enterprise-ontology-capability-framing.md`](./adr-enterprise-ontology-capability-framing.md) — [#217](https://github.com/ronsse/trellis-ai/issues/217), the umbrella ADR that frames EG/Trellis as adjacent capabilities; this bridge is one piece under it.
...
- [`./adr-query-history-promotion.md`](./adr-query-history-promotion.md) — [#218](https://github.com/ronsse/trellis-ai/issues/218). The Trellis→EG candidate handoff (§2.2) is the consumer of the promotion path; this ADR defines the wire shape of a candidate, that ADR defines the review gate that produces it.
...
An enterprise graph platform — "the EG" in this ADR: a governed enterprise graph external to Trellis — and Trellis solve adjacent problems. The EG is the system of record for **cross-domain traversal, accountability, provenance, and projections** over accepted enterprise facts. Trellis is the **agent-facing memory and retrieval layer** — traces, context packs, observations, measurements, feedback attribution, all behind a governed mutation pipeline.

The two systems share a graph shape (Node/Edge, provenance, time-travel) but differ on *authority*. EG holds curated, reviewed, organizationally-blessed facts. Trellis holds a wider, noisier corpus: agent experience, empirical observations, query-history patterns, and machine-inferred candidates that have **not** been blessed. The whole value of keeping them separate is that Trellis can record a candidate without that candidate becoming an enterprise truth.

Today there is **no explicit mapping or flow** between an EG-style Node/Edge/Event model and Trellis's graph/trace/document/observation model. Without a defined bridge:

- EG risks duplicating Trellis observations as accepted facts too early.
- Trellis risks ingesting EG facts without preserving *which system asserted them*.
- Query-history-derived facts risk becoming canonical edges with no review.
- Builders won't know whether a given event is a Trellis trace, an EG event, an `Observation`, or an EventLog audit row.
...
- EG is **not** the Trellis storage backend.
- Trellis is **not** responsible for EG's production RBAC or graph serving.
- Trellis observations are **not** EG accepted facts by default.
- Not every Trellis deployment must know about EG. The bridge is opt-in; a deployment with no EG is unaffected.

---

## 2. Mapping table

Each EG concept maps to one Trellis concept, with the **source-authority** rule that governs the crossing. Trellis already accepts any entity type / edge kind as an open string (see [`adr-graph-ontology.md`](./adr-graph-ontology.md) §5.1–5.2), so the mapping needs no new type registrations — it needs *conventions* on `properties` / `metadata`.

| EG concept | Trellis concept | Crossing rule |
|---|---|---|
| **Node** | `Entity` (`src/trellis/schemas/entity.py`) | Preserve EG `entity_type`, `domain`, and source refs in `properties`; preserve **source authority** in `metadata` (`source_system`, `fact_state="source_asserted"` or `"accepted"`). `node_role` defaults to `SEMANTIC` — an imported EG node is a real thing in the world, not Trellis-curated plumbing. |
...
The bridge is **asymmetric on purpose.** Authority flows one way (EG→Trellis preserves it); candidacy flows the other way (Trellis→EG earns it).

### 3.1 EG → Trellis (seed retrieval, preserve authority)

- **Accepted enterprise facts seed retrieval.** EG nodes/edges import as `Entity`/`Edge` with `fact_state` ∈ {`source_asserted`, `accepted`} so PackBuilder can surface them to agents.
- **EG projections can become Trellis context-pack sections.** A projection (§4) is a named retrieval shape; an EG-side projection result maps onto a Trellis pack section.
- **Source authority is preserved as metadata**, never flattened. An imported node keeps `source_system` and `declared_by`; Trellis does not relabel an EG-accepted fact as Trellis-authored.

EG→Trellis is a **read-seeding** crossing. Trellis never asserts authority over what it imported; it carries EG's authority forward as metadata.

### 3.2 Trellis → EG (publish only reviewed candidates)

- Agent traces and query-history analysis produce **observations, measurements, and curated candidates** inside Trellis.
- **Only reviewed candidates** cross to EG as accepted edges/nodes. The review gate is [#218](https://github.com/ronsse/trellis-ai/issues/218)'s promotion path.
- **Raw traces and raw query history are never bulk-copied into EG** as canonical facts. Trellis's trace corpus and query logs stay in Trellis; only the distilled, reviewed candidate crosses.

Trellis→EG is a **review-gated publish** crossing. The default state of anything Trellis produces is `candidate` / `inferred`, and crossing to `accepted` requires an explicit human-reviewed promotion — consistent with the non-goal that Trellis observations are not EG accepted facts by default.

```
EG  ──accepted facts──▶  Trellis retrieval        (authority preserved as metadata)
EG  ──projections────▶  Trellis pack sections

Trellis traces/obs/query-history ──distill──▶ candidate
candidate ──[#218 review gate]──▶ accepted ──publish──▶ EG
...
| `fact_state` | Meaning | Typical origin | Crosses to EG as accepted? |
|---|---|---|---|
| `source_asserted` | A source system asserted it; not yet enterprise-blessed | UC/dbt/Workday/CRM ingest | Only after EG acceptance |
| `accepted` | Enterprise-blessed fact | EG, or post-review in Trellis | Yes — it already is |
...
EG-style projections become **first-class named retrieval shapes**. A projection is a declarative contract — it does not execute traversals here; it constrains what a traversal *may* do. Like fact states, projections live in the ontology profile ([#219](https://github.com/ronsse/trellis-ai/issues/219)), not in core.
...
| `query_history_patterns` | semantic, curated | `Observation`, `Measurement`, `Dataset` | `hasObservation`, `wasDerivedFrom` | 2 | `fact_state=behavioral_evidence`; never crosses to EG without review |
...
1. **EG is not the Trellis storage backend.** Trellis keeps its own stores (`StoreRegistry`, `src/trellis/stores/`). The bridge is an import/export mapping, not a backend substitution. Trellis does not query EG at retrieval time as if it were a `GraphStore`.
2. **Trellis does not own EG RBAC or graph serving.** EG's production access control and serving stay in EG. Trellis's policy layer governs Trellis mutations only; it makes no claim over EG's RBAC.
3. **Trellis observations are not EG accepted facts by default.** The default `fact_state` for anything Trellis produces is `candidate` / `inferred` / `behavioral_evidence` / `curated` — never `accepted`. Crossing to `accepted` is review-gated ([#218](https://github.com/ronsse/trellis-ai/issues/218)).
4. **Not every deployment must know about EG.** The bridge, fact states, and projections all live in an opt-in ontology profile ([#219](https://github.com/ronsse/trellis-ai/issues/219)). A Trellis instance with no EG carries none of this and behaves exactly as it does today.
...
### 7.1 Accepted EG ownership edge imported into Trellis retrieval

EG holds an accepted edge: *Team `data-platform` owns Dataset `fct_orders`.* Imported into Trellis:
...
Entity(node_type="Dataset", node_role=NodeRole.SEMANTIC,
       metadata={"source_system": "eg", "fact_state": "accepted"})
...
Edge(edge_kind="wasAttributedTo",
     ...
     properties={"fact_state": "accepted", "source_system": "eg",
                 "declared_by": "eg-curation", "confidence": 1.0})
```

The `ownership` projection (§5) admits this edge: `fact_state=accepted`, edge kind `wasAttributedTo`, depth 1. It surfaces in an agent's pack with EG authority intact. Trellis never relabels it as Trellis-authored.

### 7.2 Trellis query-history `JoinPattern` exported as an EG candidate
...
This is `behavioral_evidence`, admitted by `query_history_patterns` (§5) but **not** by any enterprise projection. To become an EG candidate edge (`fct_orders relatedTo dim_customer`), it goes through the [#218](https://github.com/ronsse/trellis-ai/issues/218) review gate — transitioning `behavioral_evidence → candidate → accepted`. It does **not** become a canonical EG edge by being observed.

### 7.3 Trellis agent trace retained as trace/Activity, not an accepted EG edge

An agent reasons through a debugging task touching three datasets. The trace is ingested (immutable) and projects into the graph as an `Activity` node with `used` / `wasInformedBy` edges to the datasets it touched. This is agent experience: retrievable via the `agent_memory` projection, never crossing to EG as an accepted fact. The `Trace` is *not* an EG event and *not* an accepted edge — it stays Trellis-side per §2.3 and §3.2.

### 7.4 Source-authority preservation for UC / dbt / Workday / CRM
...
graph.upsert_alias(entity_id=E, source_system="crm",           raw_id="crm://orders/fct")
```
...
- [x] A design doc defines the EG ↔ Trellis mapping — §2.
- [x] Examples for: accepted EG ownership edge imported into retrieval (§7.1); query-history `JoinPattern` exported as a candidate (§7.2); agent trace retained as trace/Activity, not an accepted edge (§7.3); source-authority preservation for UC/dbt/Workday/CRM (§7.4).
...
- [x] Review/promotion is explicit before Trellis-generated candidates become accepted EG facts — §3.2, §4 (no implicit transition), deferred to [#218](https://github.com/ronsse/trellis-ai/issues/218).
...
- A no-EG deployment is unaffected (non-goal 4).

### 9.2 What this costs

- A new convention (`fact_state`, `declared_by`, `source_system` on metadata) that profiles and any EG bridge code must agree on. Documented here; enforced nowhere in core until a policy consumer demands it.
