...
**Synthetic-profile calibration on axes D + G.** Two of the master scenario's per-round knobs deviate from the production defaults so the synthetic corpus can clear the same regression thresholds a real corpus would. Both values live in [`eval/scenarios/program_convergence/scenario.py`](../../eval/scenarios/program_convergence/scenario.py) with inline `# Why:` comments cross-referencing this paragraph:

- **Axis D — `DEFAULT_OBSERVATION_BATCH = 10`** (synthetic) vs. **2** (real corpora). The Phase 2 axis-D regression asserts ≥ 10 observations per seed entity by round 25. With ~22 seeds × 10 observations / 25 rounds ≈ 8.8 observations/round needed, the synthetic scenario seeds 10/round (round-robin across seeds) to clear the threshold inside the 25-round window. Real corpora seed observations from query logs at a much lower rate (~2/round per seed); the production threshold assumes that real-corpus rate and the threshold itself is unchanged.
- **Axis G — `well_known_count_threshold = 3`** (synthetic) vs. **10** (real corpora — `RECOMMENDED_SEED_VALUES`). The well-known analyzer's count gate is the minimum number of distinct entity instances per open-string `node_type` before promotion is suggested. The synthetic graph this scenario produces is small (~22 seeds across 3 domains), so a threshold of 10 would never fire and axis G would flatline. Setting the threshold to 3 lets the gate fire on the small synthetic graph without disabling it (a value of 0 or 1 would mean "promote every type seen", which is not the property axis G measures). Real corpora keep the production value of 10; the per-scenario override is seeded into the parameter store at setup time and does not bleed into the production registry.

Neither value is the production default. Both are intentional synthetic-profile knobs, scoped to the master scenario's setup and not exported elsewhere. If the synthetic-corpus shape changes (more seeds per domain, more domains), revisit both values against the Phase 2 thresholds.
