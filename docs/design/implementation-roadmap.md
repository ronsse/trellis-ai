...
**Last updated:** 2026-08-02 (classification + retrieval hardening #278–#281 landed 2026-08-02. 2026-07-15: Productionization milestone §3.H added, roadmap reconciled. 2026-07-14: dogfood defect wave — retrieval defects #254/#262 + the auto-capture keystone #255 + #258/#259/#260/#263 landed. 2026-07-11: corpus §G.1 + conversation capture; `trellis analyze cost`; MCP-over-HTTP #252)
...
- [ ] **#194 classification enforcement, minimal slice** (`keystone`; `blocked:owner-decision` + `blocked:dep` on #256): populate `DataClassification` on write paths; PackBuilder/search filter and the mutation policy-gate deny by caller scope. Acceptance: `pytest tests/unit/ -k classification` proves (a) a restricted document is excluded from packs and search for an unscoped caller, (b) a mutation touching restricted content is denied without the scope. Caller identity already exists on REST (#242) and MCP-over-HTTP (#252); stdio-MCP / local-CLI stay trusted-local — document that boundary. This pulls one slice of tag-vocab Phase 4 (§D) ahead of its "design-partner-asks" gate (owner-approved exception) and is blocked-on #256 (Bolt→plugin extraction), which halves the enforcement surface.
  - **Undeclared prerequisite (surfaced 2026-08-26, untracked in any issue): the policy gate is not wired on any surface.** `MutationExecutor` takes `policy_gate` as an optional injected `PolicyGate` and skips Stage 2 entirely when it is `None` ([`mutate/executor.py:135`](../../src/trellis/mutate/executor.py)) — and `build_curate_executor`, the single factory every surface (CLI, REST, MCP) uses, passes only `event_log` and `handlers` ([`mutate/__init__.py:43`](../../src/trellis/mutate/__init__.py)). `DefaultPolicyGate` is exported from `trellis.mutate` but nothing constructs it in a non-test path. So the pipeline's documented "validate → **policy check** → idempotency → execute → emit" is a four-stage pipeline in production. This is not a defect in itself — gates are Protocol-based and injected by design — but #194's acceptance criterion (b), "a mutation touching restricted content is denied without the scope," cannot be met until something wires one. Scope this into #194 rather than filing it separately; the row-level `PolicyViolationError` path handlers already raise ([`executor.py:218`](../../src/trellis/mutate/executor.py)) is the pattern to extend.
