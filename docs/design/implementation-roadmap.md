...
### The August 2026 wave (2026-08-06 → 08-30) — **read the answered/implemented split**

~55 merges landed after this file's previous revision. Summarised, not restated: the
mechanism for each lives in `CLAUDE.md` (which *is* current) and in
[`autonomous-backlog.md`](./autonomous-backlog.md). **The most useful thing to know before
picking work up is which items were *answered* rather than *implemented*** — an answered
item has a verdict and must not be re-proposed as if it were open work, and this queue has
already dispatched an agent at one.

**Answered — measured, and the proposed fix refused or reshaped. Do not re-propose without new evidence.**

| Item | Verdict |
|---|---|
| **§G.4 chunk rollup** (group-by-`parent_doc_id` in `PackBuilder`) | ⛔ **Refused** — [#384](https://github.com/ronsse/trellis-ai/pull/384) (`d821094`). The phenomenon is real (16 of 37 packs, 11.7% of injected tokens); the fix is not — the extras are top-ranked, chunk excerpts are not duplicate text, and `max_tokens` is a quota so the saving does not materialise. Shipped the **instrument** (`retrieve/concentration.py`), not a fix. See §G.4 below. |
| **Seeding the graph axis** (`SemanticSeedExtractor`) | ⛔ **Refused on replay** — [#371](https://github.com/ronsse/trellis-ai/issues/371)/`99f63da`. Replayed over all 37 real production intents: **0 seeds on 37/37, 0/37 packs changed.** The axis is a recency feed unless seeded; that is now *observable* (`metadata["graph_selection"]`) rather than inferred. |
| **Noise demotion by `usage_rate`** ([#336](https://github.com/ronsse/trellis-ai/issues/336)) | ↺ **Reshaped** — `69986ac`. The rule was degenerate at the measured base rate, not miscalibrated: every threshold in (0, 0.333] flagged the same 64 of 79 items. Replaced by an evidence gate (`classify/demotion_gate.py`). |
| **Excerpt-width as a precision lever** | ⛔ **Refused** — [#359](https://github.com/ronsse/trellis-ai/issues/359)/`738cb74`. `useful_token_fraction` is a ratio, so a uniform cap moves numerator and denominator together. Width is a cost lever only. Graduated disclosure (post-walk demotion to pointers) is what shipped. |
| **Domain-tag promotion** | ⛔ **Refused on measurement** (2026-08-24). The best possible query over all 1160 domain tags reaches 69/1005 docs. Vocabulary, not keywords, is the blocker. |

**Implemented.** Grouped; each links its mechanism.

* **Governance:** the policy gate is **wired on every surface** (C1, `1e6c66e` — see §3.H.1), the policy file is one resolved path, and both `PolicyStore` / `AdvisoryStore` now *refuse to write over a file that loaded degraded* (`fb2e168`, `0d9f2a6`). `retention.prune` is a governed handler (#312, `ba1177e`); governed `redaction.apply` landed 2026-08-13 (`4e3f451`).
* **Retrieval:** graduated disclosure (#359), progressive disclosure / index packs (#305), file-scoped context (#307), capture-health banner inside served packs (#309), truncation marked with size + reason (#310), the noise boundary enforced at the collect seam (#338/#343), post-embed vector-metadata write-through (`875d924`), and **visible withholding** (#404, `d635fcc` — *MCP only*; see the ledger entry).
* **Measurement:** capture coverage (E2, `627536f`), served-context value density + pack-attributed response cost (`61533ef`), attribution decomposed and the join key no longer dropped (`a76a7dd`), advisory effect made a comparison (`8635997`).
* **Classification:** the LLM→deterministic tagging ladder (#321, `4550a51`) plus four fixes to readers of the same seam (#324–#327) — each of which had been silently reporting a constant.
* **Capture / extraction:** session auto-capture made able to write at all (#333/#334), deterministic evidence override (#308), skip-discipline prompts (#311), memory-path draft policy (#301), write-boundary rejection telemetry + `trellis analyze health` (#297).
* **CI + correctness:** the pgvector contract suite made to execute at all and wired into CI (#345, `f928943`), one enforced ruff version (#378), `updated_at` preserved across metadata-only writes (`fe843fc`, `31eff6c`), chunk rows excluded from whole-document surfaces (#396/#385).

**Still open and unblocked** — the queue lives in [`swarm-handoff.md`](./swarm-handoff.md) §6.
Top item is [#360](https://github.com/ronsse/trellis-ai/issues/360) (govern the document and
vector planes), whose "gains most of its point after C1" caveat is now satisfied.
...
| 2026-07-14 | **Dogfood defect wave landed** (PRs #267–#273): `domain=` facet routing (#254), one retrieval path through PackBuilder + `pack_id` (#262), `ENTITY_UPDATE` handler + evidence-doc seam (#260), session-delta packs (#258), near-dup suppression (#259), reconcile-on-write (#263), and the **Claude Code session auto-capture keystone** (#255) | milestone `Productionization` §3.H.0 |
| 2026-08-02 | **Classification + retrieval hardening** (PRs #278–#281): classify-on-write at ingest so documents carry `content_tags` (#280); `get_lessons` sees promoted learning precedents (#278); packs stop rendering the RRF-fused score as "relevance" (#279); refresh-path domain-drop safety (#281) | commits `5fdea75`..`bd6ae90` |

### Test suite shape

`pytest tests/unit/ -q` collects **4665 tests by default as of 2026-08-02** (5215 total; 550 backend-marked tests deselect without their env toggles). Counts drift with every merge — re-measure with `pytest tests/unit/ -q --co` rather than trusting this line, and treat CI's Tests job as the source of truth. CI runs five required checks (Tests, Lint, Type Check, OpenAPI, CodeQL) plus **Live infrastructure tests** — which since `586aee6` runs against **ephemeral service containers** (`neo4j:2025.12`, `pgvector/pgvector:pg16`), needs **no external instance and no repo secrets**, and includes the storage contract suites.

### Live test credentials — current reality

* **The AuraDB free-tier instance is GONE** (auto-deleted; DNS-dead — confirmed `NXDOMAIN` 2026-08-02, see #250). Do **not** follow older instructions that say "leave it running". CI no longer needs it. For local live Neo4j/ArcadeDB tests, run containers — the fixture header of `tests/unit/stores/test_arcadedb_graph.py` and `.github/workflows/live-infra.yml` show the exact `docker run` incantations and `TRELLIS_TEST_*` variables.
...
- [ ] **#250 credential hygiene** (operator-only, `ready`): the AuraDB instance is confirmed gone (`NXDOMAIN` 2026-07-15) — purge the dead credentials (1Password entry + Aura API client IDs `985676d4`/`d664924e`), recreate repo-root `.env` from `.env.example`, and remove/rotate the now-dead `TRELLIS_TEST_NEO4J_*` GitHub secrets (CI has been container-based since `586aee6`). Acceptance: #250 closed with its checklist ticked; `test -f .env` locally; `gh secret list --repo ronsse/trellis-ai` shows the secrets purged. **Status 2026-08-26: 3 of 4 tasks verified done** — the 1Password entry is absent from both vaults, the `TRELLIS_TEST_NEO4J_*` secrets are purged at repo / dependabot / environment scope (only `TRELLIS_TEST_PG_DSN` remains, and `live-infra.yml:86-89` proves CI uses hardcoded localhost-container values, not secrets), and `.env` is recreated from `.env.example`. **Remaining: deleting Aura API client IDs `985676d4` / `d664924e` from <https://console.neo4j.io>** — no CLI or API path exists for this, so it is irreducibly owner-only. Instance re-confirmed `NXDOMAIN` 2026-08-26.
- [ ] **#194 classification enforcement, minimal slice** (`keystone`; `blocked:owner-decision` + `blocked:dep` on #360): populate `DataClassification` on write paths; PackBuilder/search filter and the mutation policy-gate deny by caller scope. Acceptance: `pytest tests/unit/ -k classification` proves (a) a restricted document is excluded from packs and search for an unscoped caller, (b) a mutation touching restricted content is denied without the scope. Caller identity already exists on REST (#242) and MCP-over-HTTP (#252); stdio-MCP / local-CLI stay trusted-local — document that boundary. This pulls one slice of tag-vocab Phase 4 (§D) ahead of its "design-partner-asks" gate (owner-approved exception). Classification enforcement seams (collect, mutation policy gate) do **not** require Bolt backend extraction (#256); the practical prerequisite is **#360** (govern document/vector writes) so `DataClassification` can be populated and enforced on real write paths. **Blockers as of 2026-09-04: #360 (`blocked:dep`) and the owner decision (`blocked:owner-decision`).** The policy-gate prerequisite is cleared; see the bullet below.
  - **Prerequisite CLEARED 2026-08-27 — the policy gate is wired on every surface.** This bullet previously said the gate "is not wired on any surface" and that production ran "a four-stage pipeline"; **that was true when written (2026-08-26) and is now false.** C1 landed as [#370](https://github.com/ronsse/trellis-ai/pull/370) (`1e6c66e`): `build_curate_executor` — the single factory every surface (CLI, REST, MCP) uses — passes `policy_gate=build_policy_gate(registry)` **unconditionally** ([`mutate/__init__.py:66-71`](../../src/trellis/mutate/__init__.py), with the reasoning in that function's own docstring at `:47-63`). Stage 2 runs everywhere, so "is the gate wired?" has an observable answer instead of depending on file state. A deployment that has declared no policies gets an *empty* gate that is behaviourally and byte-for-byte indistinguishable from the old no-gate world (pinned by `tests/unit/mutate/test_policy_wiring.py::TestDefaultPostureIsTransparent`). **Consequence for #194: acceptance criterion (b) is now reachable** — what remains is populating `DataClassification` on write paths and writing the scope-denying policy, not building the enforcement seam. The row-level `PolicyViolationError` path ([`executor.py`](../../src/trellis/mutate/executor.py)) is still the pattern to extend. Full mechanism: `CLAUDE.md` § "Governed Mutation Pipeline".
...
|---|---|---|
| §3.H.1 — security floor | #250 credential purge (operator console) · #194 classification enforcement (owner-approved pull-forward, blocked-on #360) | `ready` now — see §3.H |
