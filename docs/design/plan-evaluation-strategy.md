...
Phases 1 + 2 of [`plan-neo4j-hardening.md`](./plan-neo4j-hardening.md) are
done. Trellis is now a credible "blessed Neo4j default for both local and
cloud" with Postgres as a real (migration-supported) alternative. 2493
unit + integration tests pass; live-tested against AuraDB Free and Neon
Postgres.

What we don't have: **evidence of how the system behaves on real-shaped
workloads**. Every Phase 3 deferred item in the hardening plan is gated on
"a workload signal we don't have yet." The next step is to *generate* that
signal — build evaluation infrastructure that runs the system against
realistic data and tells us:
...
* Neo4j hardening Phase 1 + 2 complete: driver lifecycle / pool sharing / shutdown / connectivity check / vector-index wait / Community-race documentation / combined-plane config / onboarding docs / `trellis admin migrate-graph`.
...
* Test suite: 2493 unit + integration / 0 skipped / 0 failed with all extras + `.env` loaded.
...
1. **Land the eval skeleton** (~1 day): `eval/` directory, `runner.py`, `pyproject.toml` exclusion, `tests/` smoke test that exercises a no-op scenario, doc page in `docs/design/eval.md` cross-linked from this plan.
