...
| Trusting a green local `pytest` run | **the shared `.venv` is not CI's dependency set.** click 8.3.2 / structlog 25.5.0 locally vs **8.5.0 / 26.1.0** in CI. This produced **109 CI failures across 23 directories** on a PR whose author had a fully green local run of CI's exact command | check against **`/mnt/ssd/trellis-worktrees/.ci-venv`** (CI-resolved versions) before opening a PR. It reproduced that failure from a single test file in ~1s. **Do not modify the shared `.venv`** — it is production's editable install. Caveat: `.ci-venv` is authoritative for the click/structlog problem it was built for, and is only authoritative for lint or typecheck when it *is* CI's gate environment — which is now checkable rather than assumed: run `make env-check` ([#398](https://github.com/ronsse/trellis-ai/issues/398)), and see [#378](https://github.com/ronsse/trellis-ai/issues/378) for why the pin drift matters |
