...
| A worktree using the main tree's `.venv` | `import trellis` resolves to **main's** code — but **only for bare `python`**, not for the `pytest` driver. **Every subprocess a test spawns is a bare `python`.** | `pyproject.toml` sets `pythonpath = ["src", "."]`, so **pytest already resolves to the worktree's `src`** with or without the env var. Verified empirically 2026-08-28 by marker probe in a scratch worktree: pytest → worktree `src` either way; bare `python -c "import trellis"` → **main's** `src`. So prefix bare `python`/scripts with `PYTHONPATH=<worktree>/src`; for the pytest *driver* it is harmless but not load-bearing. **But every subprocess a test spawns is a bare `python`** — #431: `tests/integration/cli/test_subprocess_smoke.py` is the only place in the suite that parses a real process's stdout, and from a worktree it was reporting green about `main`. #428 pinned the three fixtures; #430/#431 shipped the enforcer — `tests/unit/test_subprocess_pythonpath_rule.py` (every launch site is pinned or boundary-guarded, with two independent scans of the population) plus a `test_the_subprocess_under_test_is_this_checkout` in both subprocess directories. **This entry previously claimed pytest was affected and was propagated to six agents before being measured** — the original probe tested bare `python` and the result was generalised; the correction then over-swung to "pytest is fine", which is true only of the driver. |
