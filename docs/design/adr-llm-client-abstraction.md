...
- **Per-consumer tier routing** — `build_llm_client(*, consumer=...)` resolves `llm.tiers` / `llm.routes` through [`llm/routing.py`](../../src/trellis/llm/routing.py) before constructing anything, so one deployment can run reconciliation on a thinking model and session capture on a cheap one. This is the wiring the Phase 3 deferred note above left open, arriving at the *registry* rather than at a workflow engine: the consumer is the unit an operator assigns a model to, and it outlived `WorkflowEngine.TierConfig`. Two properties are load-bearing and neither is obvious from the signature. **`consumer` is keyword-only and defaults to `None`, and `None` means the parent block** — so a call site that forgets it is indistinguishable from a pre-tier deployment: nothing raises, nothing logs, and `trellis admin llm-routes` keeps printing a route that reaches no code. `tests/unit/test_llm_consumer_rule.py` derives the invariant by AST over `src/` instead of trusting a roster, and pins the default it depends on. And **a routed consumer never falls back** — not to the parent block and not to the `OPENAI_API_KEY` env path the MCP server still keeps for unrouted consumers — because a fallback here runs the expensive consumer on the cheap model and reports success. See [playbooks.md Playbook 12](../agent-guide/playbooks.md) for the four resolution rules and the `TRELLIS_RECONCILE_MODEL` correction (it is an event label, never a model selector).
