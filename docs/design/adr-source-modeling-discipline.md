...
- [`./adr-column-leaf-modeling-guardrails.md`](./adr-column-leaf-modeling-guardrails.md) — **the policy authority.** Default rule (§2), exception criteria (§4), node requirements (§5), maintenance-cost argument (§6), EGP reconciliation (§7), and the §8 follow-up plan this ADR's advisory-warning decision takes the gate on.
