...
|---|---|---|---|---|
| **`structural`** | Machine-generated plumbing that is not standalone memory and is reproducible from somewhere outside this graph. Exists to support its parent, or to be traversable — not to be retrieved independently. | Columns *when* column-level lineage is a real requirement; function parameters when call-graph analysis is needed; file lines when diff-based retrieval matters; the `tool:<slug>` nodes trace extraction mints; the per-invocation `Activity` the meta-recorder writes. | **Filtered out** of retrieval by default. Consumers must explicitly opt in. | Still SCD Type 2, but property histories are usually redundant with the source system's history. |
...
### What `structural` covers, and why the wording was widened (#375)

The role used to be described as "regenerated from source (e.g., columns, function parameters)". That named one *example* of a machine-generated non-standalone row and was read as the definition, which left a second shape homeless: rows that **restate an Operational-Plane fact**. The meta-recorder writes one `Activity` per analyzer invocation; on the reference deployment that was **200 of 1000 current nodes**, all `cli.*`-named, with **zero** citations across 190 graded graph servings — never standalone memory, and fully reconstructible from the event log and trace store, but not regenerated from any source system.

The definition now reads at the altitude the behaviour already sits at: *machine-generated plumbing that is not standalone memory and is reproducible from somewhere outside this graph*. Both shapes want the same single bit — "is this row standalone-discoverable?" — and a fourth role would have obliged every backend, filter, doc and consumer to learn a value whose retrieval semantics are identical to `structural`.
