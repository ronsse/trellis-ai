...
```json
{"status": "ok", "scanned": 240, "refreshed": 31, "skipped_fresh": 200, "skipped_no_signal": 6, "skipped_missing_content": 3, "dry_run": false, "include_domain": false, "item_ids_refreshed": ["doc-1", "doc-2"]}
```
...
Everything else is configured by the `TRELLIS_CAPTURE_*` environment variables documented in [session-auto-capture.md](session-auto-capture.md). **Requires a distillation judge** (an `llm:` block): distillation fail-closes, so a sweep without one captures nothing — the command exits non-zero rather than reporting a clean no-op, and exits non-zero again when `sessions_judge_unavailable` is non-empty because the judge went away mid-sweep.
