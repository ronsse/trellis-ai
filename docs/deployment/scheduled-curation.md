...
**A note on `worker curate`'s exit code:** it is a function of the reported `status` and nothing else. `ok` exits `0`; `degraded` (the advisory file could not be read) and `stale` (another process wrote it mid-cycle) both exit `5` — the canonical *store / backend / config* code from [`../design/adr-cli-exit-codes.md`](../design/adr-cli-exit-codes.md) — on the text surface and the JSON surface alike. The other stages still ran and their counts are still in the payload — the non-zero code says a write did not land, not that the cycle did nothing.

> **Changed:** these two conditions exited `2` before [#489](https://github.com/ronsse/trellis-ai/issues/489), and so did `trellis analyze generate-advisories` and `trellis analyze advisory-effectiveness` meeting the same file. That was wrong in exactly the way `2` invites: `2` means *"the input failed a check — fix it and retry"*, and a wrapper retrying with corrected arguments against an `advisories.json` it cannot parse loops forever. `5` says what is true — the deployment's state is wrong and a human has to change it — and it is what `trellis policy list` already exits meeting the sibling `policies.json` damaged the same way. **If your wrapper branches on `2` for any of these commands, change it to `5`.** A wrapper that only tests for non-zero (`set -e`, `if ! trellis …`) needs no change.
