# Silent-fallback audit — 2026-05-12 baseline (Wave 4 Phase 7)

> **Purpose:** This file is the "before" snapshot for the C2 Phase 7 verification
> defined in `plan-cleanup-silent-fallbacks` (Phase 7). It captures the
> silent-fallback DEFECT inventory at current `main` HEAD (date: **2026-05-12**),
> before Phases 1.5/2/3/4/5/6 land. Wave 4 will diff the "after" run against this
> baseline to confirm the cleanup actually reduced DEFECT counts in the expected
> modules.
>
> **Comparison point:** the original 2026-05 baseline is
> [`audit/silent_fallbacks_2026-05.md`](silent_fallbacks_2026-05.md) (153 candidates,
> 112 DEFECTs). This file is regenerated from the same script with no source-tree
> edits.
>
> **Script mode note (Followup 2, 2026-05-13):** this baseline was generated
> in **literal-only** mode — the script counted an ``except`` block as
> non-silent only when its body contained a literal ``raise``. The helper
> awareness shipped in `feat/audit-script-helper-chain-awareness` is now the
> default; it also recognises calls to ``_raise_*`` / ``raise_*`` helpers,
> functions annotated ``NoReturn``, intra-module functions whose body ends
> in ``raise``, and stack-abort calls like ``sys.exit`` / ``typer.Exit`` /
> ``click.Abort``. To reproduce the numbers in *this* file exactly, re-run
> with ``--literal-only``. The "after" snapshot Wave 4 D3 produces should
> use the new default (helper-aware) so the MCP layer — which routes 19 of
> 31 PR #119 sites through ``_raise_*`` helpers — is counted correctly.
...
- **Total candidate `except` sites flagged:** **156**

## Bucket totals

- **DEFECT**: 113 (72.4%)
- **GRACEFUL-DEGRADATION**: 8 (5.1%)
- **GUARD**: 35 (22.4%)
- **TEST-ONLY**: 0 (0.0%)
...
|---|---:|---:|---:|---:|---:|
| `trellis/` | 121 | 89 | 7 | 25 | 0 |
| `trellis_cli/` | 16 | 10 | 0 | 6 | 0 |
...
#### `trellis_cli/admin.py` (11 sites — DEFECT=5, GUARD=6)

- **line 457** — bucket=**DEFECT**, pattern=`other`, catch=`Exception`, fn=`graph_health`
...
- **line 505** — bucket=**DEFECT**, pattern=`other`, catch=`Exception`, fn=`graph_health`
...
- **line 627** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_ensure_gitignore`
...
- **line 753** — bucket=**DEFECT**, pattern=`return-empty`, catch=`specific`, fn=`_memory_prompt_available`
...
- **line 1211** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_check_healthz`
...
- **line 1227** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_check_readyz`
...
- **line 1246** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_check_auth_rejects_missing`
...
- **line 1262** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_check_auth_accepts_valid`
...
- **line 1278** — bucket=**GUARD**, pattern=`other`, catch=`specific`, fn=`_check_metrics`
...
- **line 1311** — bucket=**DEFECT**, pattern=`return-empty`, catch=`specific`, fn=`_safe_json`
...
- **line 1652** — bucket=**DEFECT**, pattern=`return-empty`, catch=`specific`, fn=`_evidence_span_days`
