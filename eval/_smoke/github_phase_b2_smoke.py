"""Phase B-2 smoke — GitHub corpus convergence at configurable rounds.

Run via::

    op run --env-file=.env -- uv run python -m eval._smoke.github_phase_b2_smoke
    op run --env-file=.env -- uv run python -m eval._smoke.github_phase_b2_smoke --rounds 100 --feedback-batch-size 10

Cost: under $0.001 per run (embeddings only).
"""
...
from eval.scenarios.github_corpus_convergence.scenario import run as run_scenario
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase B-2 smoke runner")
    parser.add_argument("--rounds", type=int, default=12)
    parser.add_argument("--feedback-batch-size", type=int, default=4)
    parser.add_argument(
        "--no-graphsearch",
        action="store_true",
        help="Disable SeededGraphSearch — keyword + semantic only",
    )
    ...
    for k in sorted(report.metrics):
        if k.startswith("per_difficulty.") and k.endswith(".success_rate"):
            print(f"  {k}: {report.metrics[k]}")
    print()
    print("Seed extraction:")
    for k in sorted(report.metrics):
        if k.startswith("seed_extraction."):
            print(f"  {k}: {report.metrics[k]}")
    print()
