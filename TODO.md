...
- [ ] **Axis C semantics — tighten from proxy to plan's prose definition.** Phase 0 wires axis C as "advisories whose `scope` intersects domains where recent rounds succeeded" because the dual-loop scenarios don't carry advisory-presentation telemetry per pack item. The plan's prose definition is "advisories whose recommendation was *followed* AND outcome=success". Tightening requires either adding `advisory_id` provenance to `PackItem.metadata` or a parallel `injected_advisory_ids` payload on `PACK_ASSEMBLED`. Phase 2's regression threshold for axis C will need to grade against the tightened metric. [severity: M]
