...
- [ ] **Axis G machinery gap — master scenario bypasses governed mutation pipeline.** The well-known analyzer counts distinct extractors per type via `MUTATION_EXECUTED` events that carry `requested_by`. The master scenario writes nodes directly via `graph_store.upsert_node()` / `upsert_edge()`, bypassing the pipeline that emits those events. Result: `extractors_by_entity_type` is always empty, the `distinct_extractors` gate trips, axis G stays at 0 regardless of `well_known_count_threshold`. Fix: either route the scenario's node writes through `MutationExecutor`, or synthesise `MUTATION_EXECUTED` events directly into the EventLog like `_seed_extraction_failures` does for `EXTRACTION_FAILED`. Bundle with Axis A threshold fix in a Phase 5. [severity: M]
