...
- [ ] **Renderer NOT wired into master scenario's `run()`.** Phase 3 ships the renderer but `eval/scenarios/program_convergence/scenario.py` doesn't call it, so a `uv run python -m eval.runner --scenario program_convergence` invocation doesn't produce a chart automatically. Also: master scenario doesn't expose `_MultiAxisStats` on `ScenarioReport` (only flattened metrics dict), so a Python-side caller has to re-build stats from metrics or get a `stats` reference some other way. Decide between (a) wire the renderer into `scenario.run()` with an optional `render_chart: bool = False` kwarg, (b) expose `_MultiAxisStats` on `ScenarioReport` so post-hoc rendering is one-liner. [severity: M]
- [ ] **No CHANGELOG entry for matplotlib dev dep.** Phase 3 added `matplotlib>=3.8` to `[project.optional-dependencies] dev` (the Phase 0/2 plan claimed it was already there — it wasn't). Adding a one-line "matplotlib dev dep for `eval/reports/program_convergence_chart.py`" to the unreleased section of `CHANGELOG.md` is good hygiene. [severity: M]
