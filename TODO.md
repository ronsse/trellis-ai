...
- [x] **Investigate `(source_system, raw_id)` collision semantics for user-managed aliases** — captured in [`docs/design/adr-alias-resolution.md`](docs/design/adr-alias-resolution.md) (Proposed). Documents the SCD-2 rebind footgun, sanctions the demo loader's name-as-raw-id pattern, and pins three deferred shapes (namespacing, strict-mode upsert, detection-only event). Status flips to Accepted once the contract test in §2.1 lands.
- [ ] **Contract test for the `(source_system, raw_id)` rebind semantic** — `tests/unit/stores/contracts/graph_store.py`. Re-upserting with the same `(source_system, raw_id)` and a *different* `entity_id` MUST close the prior row and open a new one pointing at the new entity. Pins `upsert_alias` against silent backend drift to "ignore" or "raise" on collision. Required for [`adr-alias-resolution.md`](docs/design/adr-alias-resolution.md) §2.1 → Accepted.
