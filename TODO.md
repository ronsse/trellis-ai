...
- [ ] **Eval-blocked retrieval gaps after Phase E** — surfaced 2026-05-07 as the residue from the Phase D + E phrase-seed and category-seed wins (commits `1d0dc1b`, `44b3d17`, `746c5b1`). Six of seven github skills and six of eight dbt skills now sit at 100% across the convergence scenarios. Two skills remain:
  - **github `multi_pr_series` Q1 (47%)** — intent "eval-framework Phase 1 through Phase 4 PRs that shipped scenarios 5.1, 5.2, 5.3" requires PRs #29-#32 whose titles all start "Eval Phase N" but `Phase 1` / `Phase 2` are non-unique across PR titles, so the title-phrase index rejects them. Two fixes available: (a) extend `_title_tokens_and_phrases` in [`eval/corpora/github_trellis/loader.py`](eval/corpora/github_trellis/loader.py) to also index unique tokens from PR *bodies* (not just titles) — same uniqueness filter, broader source, will pull `harness skeleton` / `multi-backend equivalence` / `synthetic traces` / `populated-graph performance` from #29-#32's bodies; (b) LLM-driven intent rewriting that expands "Phase 1 through Phase 4 PRs" into `["Eval Phase 1", "Eval Phase 2", ...]` before seed extraction. (a) is deterministic and ≈40 LOC; (b) is more flexible but adds an LLM cost per query. Try (a) first.
...
**Original top-5 + 1.2 + 2.2 + 2.4 + 3.1 + 3.3 + 3.4 + 4.2 + 4.3 + 4.4 complete.** Remaining open gaps: **1.3** (malformed-trace validation), **3.5** (stale importance scores), **4.5** (schema fingerprint on substrate switch).
...
- [ ] **3.5 Importance scores are stale; no temporal decay on scores themselves.** Recency decay exists on retrieval *scoring* (half-life 30d, floor 0.3) but the per-item `importance` — set at ingest or via [`classify.noise_tags`](src/trellis/classify/) — has no `score_age` and no refresh job. A document scored 6 months ago ranks equally to one scored today, unless explicitly re-tagged. Lands harder in alert/dashboard domains than in stable-doc domains.
...
- [ ] **4.5 No schema fingerprint on substrate switch.** [`stores/registry.py:56-108`](src/trellis/stores/registry.py:56) SQLite ↔ Postgres (or future SurrealDB) migrations have no validation layer. Columns added in one backend silently default to NULL when read from the other. No version stamp, no schema hash. Not urgent today because migrations are rare, but lands right when the blessed-substrate decision (§ Substrate) is ready to move.
