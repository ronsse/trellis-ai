...
- [ ] **Chart renderer hardcodes `figsize=(15.0, 11.0)` and `dpi=100`.** For very long runs (50+ rounds with multi-segment x-axes) the per-subplot tick density may get cramped. Consider exposing these as optional kwargs once Phase 2's regression suite starts firing in CI with 50-round configurations. Defaults look fine on the 20-round smoke render. [severity: L]
...
- [ ] **`_render_chart()` writes to hard-coded `eval/reports/` path.** Operators running the scenario from a different CWD will write the PNG to a wrong relative path or fail. Thread `output_dir` as a `run()` kwarg or compute from `__file__`. Leaving as-is until a real operator hits the friction. [severity: L]
