...
- [ ] **File-only JSONL → `learning.scoring` bridge — ADR-shaped if it's needed at all** — surfaced 2026-04-29 while landing [`plan-evaluation-strategy.md` §5.5.2 row 2](docs/design/plan-evaluation-strategy.md). The EventLog → learning-observation bridge landed in [`src/trellis/learning/pack_observations.py`](src/trellis/learning/pack_observations.py), but a *file-only* variant — read `pack_feedback.jsonl` without an EventLog and still drive promotion — is **structurally underspecified**. [`PackFeedback`](src/trellis/feedback/models.py) carries `items_served` (item_ids only) and `items_referenced` but **not** the per-item `item_type` / `source_strategy` / `category` that [`analyze_learning_observations`](src/trellis/learning/scoring.py) needs to produce promotion candidates. To close: either (a) extend `PackFeedback` to capture per-item shape (likely bloats every JSONL row), (b) add a sibling `pack_assembly.jsonl` written by [`pack_builder._emit_telemetry`](src/trellis/retrieve/pack_builder.py) and join the two file logs at promotion time, or (c) declare the JSONL path *demote-only* (its current de-facto state) and remove the dual-path framing from CLAUDE.md. **Validate before designing:** is anyone actually running file-only? Today the dual-loop framing is in CLAUDE.md but no production caller skips the EventLog. If the file-only path has no real consumer it should be (c) — delete the framing, not extend the schema. Defer until a deployment actually drops the EventLog.
