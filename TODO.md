...
**Original top-5 + 2.2 + 2.4 + 3.1 + 3.4 + 4.2 + 4.3 + 4.4 complete.** Remaining open gaps: **1.2** (silent classifier-mode switch), **1.3** (malformed-trace validation), **3.3** (tag filters AND-only), **3.5** (stale importance scores), **4.5** (schema fingerprint on substrate switch).
...
- [ ] **1.2 Silent classifier-mode switch.** [`classify/pipeline.py:43`](src/trellis/classify/pipeline.py:43) The `mode` property is stateless and derived from `llm_classifier is not None`. Toggling the LLM client at runtime silently changes behavior for the same content — no `CLASSIFIER_MODE_CHANGED` event, no per-item record of which mode produced the tags. Reclassification comparisons become impossible.
...
- [ ] **3.3 Tag filters are AND-only; no OR, no negation.** [`retrieve/pack_builder.py:676-694`](src/trellis/retrieve/pack_builder.py:676) `_build_filters()` merges `tag_filters` into `content_tags` with implicit AND. Can't express `domain in [A, B]`, `signal_quality != noise`, or `content_type = tutorial OR reference`. Callers work around by multiple calls + union, losing unified budget enforcement.
