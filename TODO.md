...
- [ ] **Investigate `(source_system, raw_id)` collision semantics for user-managed aliases** — the `LOCAL_SOURCE_SYSTEM` convention assumes raw_ids are unique across all entity types in a Trellis instance. If two entities ever share a raw_id under `"local"`, SCD-2 supersedes the older alias and silently re-points it. Demo data avoids this by construction. For real users adopting the convention, decide: namespace per type (`svc:foo`), document the constraint, or add a collision-detection guard at upsert time.
