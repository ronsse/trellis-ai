...
**Original top-5 + 2.4 complete.** Next candidates: **4.3** (extractor tier fallback telemetry), **3.1** (token-budget boundary accuracy), **4.2** (temporal graph compaction), **3.4** (close loop on rejection telemetry).
...
- [ ] **2.2 No automatic rollback for promoted parameters.** [`learning/tuners/promotion.py`](src/trellis/learning/tuners/promotion.py) `promote_proposal()` writes a new `ParameterSet` once policy gates pass. Nothing watches post-promotion outcomes. A parameter that wins at n=10 and degrades at n=100 can only be undone by a new manual proposal. No `PARAMETERS_DEGRADED` signal, no automatic demotion, no shadow-evaluation mode.
...
- [ ] **3.1 Token-budget estimate is off at boundaries.** [`retrieve/pack_builder.py:640-673`](src/trellis/retrieve/pack_builder.py:640) `estimate_tokens()` is `len(text) // 4` — a linear approximation. Actual tokenization varies 5–20% depending on content (code, JSON, URL-heavy text). `_apply_token_budget_tracked()` is strict-`<=` at the boundary, so packs can exceed real context windows or fall short. No post-hoc validation against a real tokenizer even in telemetry.
...
- [ ] **3.4 `PACK_ASSEMBLED` telemetry produced but not closed-loop consumed.** [`retrieve/pack_builder.py:437-475`](src/trellis/retrieve/pack_builder.py:437) Payload carries `rejected_items`, `budget_trace`, `strategies_used` — rich signal about where packs fall short. `AdvisoryGenerator` only reads `injected_item_ids` and `strategies`, ignoring rejection reasons and budget overflow. No tuner watches "budget hit rate" to propose budget or ranking adjustments.
...
- [ ] **4.2 Temporal versioning has no retention or compaction.** [`stores/base/graph.py`](src/trellis/stores/base/graph.py) SCD Type 2 `valid_from` / `valid_to` rows accumulate indefinitely. Hot nodes that update frequently can reach 1000s of versions in months. `as_of` queries degrade (full scan with range filter), storage bloats. No `compact_old_versions()`, no `archive_before(ts)`, no retention policy.
- [ ] **4.3 Extractor tier fallback is opaque.** [`extract/base.py:35-46`](src/trellis/extract/base.py:35) Dispatcher enforces `DETERMINISTIC > HYBRID > LLM` priority with `allow_llm_fallback=False` default, but there is no `EXTRACTOR_FALLBACK` event when a deterministic extractor fails and an LLM-tier takes over. "Graduation tracking" (claimed in Phase 2 docs as a future capability) needs this telemetry as its substrate.
- [ ] **4.4 Blob-store GC absent.** [`stores/registry.py:74`](src/trellis/stores/registry.py:74) `local` and `s3` blob backends register fine but there is no TTL field, no `delete_after()`, no sweeper. Large extraction artifacts (full manifests, raw uploads) leak indefinitely. Manual cleanup only. Bounds on S3 cost are operational, not enforced.
