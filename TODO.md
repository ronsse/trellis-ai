...
- [ ] **Phase 8.1 (conditional) — `advisory_store._load` / `policy_store._load` revisit.** C2 Phase 8 (#140) classified both `_load` paths as ANNOTATE-GRACEFUL: silent-empty-on-corrupted-store is acceptable today because deny-by-default downstream protection (PolicyEngine fails closed, AdvisoryGenerator skips when list is empty) means a corrupted store produces an obviously-broken safe state rather than a stealth-permissive one. **Conditional trigger:** if a real misconfig incident occurs that the deny-by-default downstream protection doesn't catch in time (e.g., a deployment ships with a corrupted policy store and no operator notices until permissive behavior is observed in production), promote to LOUD — raise `StoreLoadError` from `_load`, fail fast at registry construction. Until then, the annotation is sufficient.
...
- [ ] **`test_structlog_routes_to_stderr_not_stdout` pre-existing flake.** [`tests/integration/cli/test_subprocess_smoke.py::test_structlog_routes_to_stderr_not_stdout`](tests/integration/cli/test_subprocess_smoke.py) fails on base SHA `2ca9584` (origin/main HEAD as of 2026-05-15) with empty stderr — the `store_instantiated` event never reaches stderr. Likely a logging-configuration ordering bug in `configure_stderr_logging` when invoked via `admin stats` as a subprocess. Pre-existing; not regressed by any swarm unit. Worth a diagnostic pass. Surfaced by C1 swarm Unit A. [severity: M]
