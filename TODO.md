...
- [ ] **MCP `record_feedback` never writes `pack_feedback.jsonl`.** [severity: M] [`mcp/server.py:1009`](src/trellis/mcp/server.py) emits `FEEDBACK_RECORDED` only; the JSONL audit log is written solely by `trellis.feedback.recording.record_feedback()` (API/SDK path), which nothing on skynet uses — the file does not exist anywhere in prod, so the nightly `--reconcile-first` has nothing to backfill. Wire the MCP tool through the same recording helper (or drop `--reconcile-first` from the cron and note MCP-only deployments don't get the file).
...
- [ ] **Pre-existing `PLR2004` ruff violation in `eval/scenarios/program_convergence/scenario.py:413` (magic value 0.5).** The `eval/` tree as a whole has 66 ruff errors that `make lint` (which only checks `src/ tests/`) doesn't gate against. If we want `eval/` linted, that's a separate PR — fix the 66 errors and add `eval/` to the Makefile's `lint` target. [severity: L]
