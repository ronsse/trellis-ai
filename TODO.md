...
- [ ] **Close the LLM→deterministic ladder for tagging** ([#321](https://github.com/ronsse/trellis-ai/issues/321)). The north-star loop — LLM bootstraps, deterministic inherits, LLM switches off for what was learned — does not exist for the tagging layer, and neither does its input: measured 2026-08-22, **all 968 classified prod docs are `classified_mode: "ingestion"`; zero LLM classifications have ever been persisted**. Phase 1 = shadow-mode LLM tags (persisted, never served — which also sidesteps the disjoint `content_type` vocabularies: deterministic emits `error-resolution`/`procedure`/`code`, the LLM emits `DEFAULT_CLASSIFICATIONS`). Phase 2 = a domain-keyword promotion analyzer modelled on [`learning/schema_evolution.py`](src/trellis/learning/schema_evolution.py) (read-only, `ParameterRegistry` thresholds, idempotent, filters its own writes), emitting into the `config.yaml` `classify.domain_keywords` seam that `KeywordDomainClassifier` already merges over its built-ins. `domain` stays **surface-only** — it is the one facet that hard-excludes, so a wrong promoted keyword hides content (the #282 failure); ranking-only facets can auto-promote under the [`tuners/auto_promote.py`](src/trellis/learning/tuners/auto_promote.py) gate (30 samples / 0.25 effect, with rollback). Companion to [#264](https://github.com/ronsse/trellis-ai/issues/264) (the general judged-op logging substrate) and [#306](https://github.com/ronsse/trellis-ai/issues/306).
