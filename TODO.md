...
- [ ] **Axis A threshold mis-calibrated for synthetic corpus.** Plan §4.2 says "delta ≥ 0.15" (15-point lift round-5 → round-50). Synthetic corpus starts at ~0.94 pack quality and converges to ~1.0, leaving only ~6 points of headroom — observed delta is 0.0545. Fix options: (a) lower the threshold for synthetic profile, (b) harden the corpus generator to produce a lower starting baseline (more distractor docs, noisier ground truth). Captured by current `regress` Finding on every run. [severity: M]
